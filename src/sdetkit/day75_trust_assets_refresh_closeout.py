...
_DAY74_BOARD_PATH = "docs/artifacts/day74-distribution-scaling-closeout-pack/day74-delivery-board.md"
...
_REQUIRED_DATA_KEYS = ['"plan_id"', '"trust_surfaces"', '"baseline"', '"target"', '"confidence"', '"owner"']
...
def build_day75_trust_assets_refresh_closeout_summary(root: Path) -> dict[str, Any]:
    ...
    checks: list[dict[str, Any]] = [
        {"check_id": "readme_day75_command", "weight": 7, "passed": ("day75-trust-assets-refresh-closeout" in readme_text), "evidence": "README day75 command lane"},
        {
            "check_id": "docs_index_day75_links",
            "weight": 8,
            "passed": ("day-75-big-upgrade-report.md" in docs_index_text and "integrations-day75-trust-assets-refresh-closeout.md" in docs_index_text),
            "evidence": "day-75-big-upgrade-report.md + integrations-day75-trust-assets-refresh-closeout.md",
        },
        {"check_id": "top10_day75_alignment", "weight": 5, "passed": ("Day 75" in top10_text and "Day 76" in top10_text), "evidence": "Day 75 + Day 76 strategy chain"},
        {"check_id": "day74_summary_present", "weight": 10, "passed": day74_summary.exists(), "evidence": str(day74_summary)},
        {"check_id": "day74_delivery_board_present", "weight": 7, "passed": day74_board.exists(), "evidence": str(day74_board)},
        {
            "check_id": "day74_quality_floor",
            "weight": 13,
            "passed": day74_strict and day74_score >= 95,
            "evidence": {"day74_score": day74_score, "strict_pass": day74_strict, "day74_checks": day74_check_count},
        },
        {
        ...
        {"check_id": "trust_refresh_plan_data_present", "weight": 10, "passed": not missing_plan_keys, "evidence": missing_plan_keys or _TRUST_PLAN_PATH},
    ]
    ...
    if day74_strict:
        ...
        handoff_actions.append("Re-run Day 74 closeout command and restore strict baseline before Day 75 lock.")

    if board_count >= 5 and board_has_day74:
        wins.append(f"Day 74 delivery board integrity validated with {board_count} checklist items.")
    else:
        misses.append("Day 74 delivery board integrity is incomplete (needs >=5 items and Day 74 anchors).")
    ...
    if not missing_plan_keys:
        ...
        handoff_actions.append("Update .day75-trust-assets-refresh-plan.json to restore required keys.")

    if not failed and not critical_failures:
        wins.append("Day 75 trust assets refresh closeout lane is fully complete and ready for Day 76 contributor recognition.")
    ...
    return {
        ...
        "inputs": {
            ...
            "top10": _TOP10_PATH,
            "day74_summary": str(day74_summary.relative_to(root)) if day74_summary.exists() else str(day74_summary),
            "day74_delivery_board": str(day74_board.relative_to(root)) if day74_board.exists() else str(day74_board),
            "trust_refresh_plan": _TRUST_PLAN_PATH,
        },
        "checks": checks,
        "rollup": {"day74_activation_score": day74_score, "day74_checks": day74_check_count, "day74_delivery_board_items": board_count},
        "summary": {
...
def _emit_pack(root: Path, pack_dir: Path, payload: dict[str, Any]) -> None:
    ...
    _write(target / "day75-trust-assets-refresh-closeout-summary.json", json.dumps(payload, indent=2) + "\n")
    ...
    _write(target / "day75-delivery-board.md", "\n".join(["# Day 75 delivery board", *_REQUIRED_DELIVERY_BOARD_LINES]) + "\n")
    _write(target / "day75-validation-commands.md", "# Day 75 validation commands\n\n```bash\n" + "\n".join(_EXECUTION_COMMANDS) + "\n```\n")
...
def _execute_commands(root: Path, evidence_dir: Path) -> None:
    ...
    _write(out_dir / "day75-execution-summary.json", json.dumps({"total_commands": len(events), "commands": events}, indent=2) + "\n")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if ns.execute:
        evidence_dir = Path(ns.evidence_dir) if ns.evidence_dir else Path("docs/artifacts/day75-trust-assets-refresh-closeout-pack/evidence")
