...
def build_day59_phase3_preplan_closeout_summary(root: Path) -> dict[str, Any]:
    ...
    checks: list[dict[str, Any]] = [
        {"check_id": "readme_day59_command", "weight": 7, "passed": ("day59-phase3-preplan-closeout" in readme_text), "evidence": "README day59 command lane"},
        {"check_id": "docs_index_day59_links", "weight": 8, "passed": ("day-59-big-upgrade-report.md" in docs_index_text and "integrations-day59-phase3-preplan-closeout.md" in docs_index_text), "evidence": "day-59-big-upgrade-report.md + integrations-day59-phase3-preplan-closeout.md"},
        {"check_id": "top10_day59_alignment", "weight": 5, "passed": ("Day 59" in top10_text and "Day 60" in top10_text), "evidence": "Day 59 + Day 60 strategy chain"},
        {"check_id": "day58_summary_present", "weight": 10, "passed": day58_summary.exists(), "evidence": str(day58_summary)},
        {"check_id": "day58_delivery_board_present", "weight": 8, "passed": day58_board.exists(), "evidence": str(day58_board)},
        {"check_id": "day58_quality_floor", "weight": 15, "passed": day58_strict and day58_score >= 95, "evidence": {"day58_score": day58_score, "strict_pass": day58_strict, "day58_checks": day58_check_count}},
        {"check_id": "day58_board_integrity", "weight": 7, "passed": board_count >= 5 and board_has_day58, "evidence": {"board_items": board_count, "contains_day58": board_has_day58}},
    ...
    if day58_strict:
        ...
        handoff_actions.append("Re-run Day 58 Phase-2 hardening closeout command and restore strict baseline before Day 59 lock.")

    if board_count >= 5 and board_has_day58:
        wins.append(f"Day 58 delivery board integrity validated with {board_count} checklist items.")
    else:
        misses.append("Day 58 delivery board integrity is incomplete (needs >=5 items and Day 58 anchors).")
    ...
    if not missing_contract_lines and not missing_quality_lines and not missing_board_items:
        ...
        misses.append("Phase-3 pre-plan contract, quality checklist, or delivery board entries are missing.")
        handoff_actions.append("Complete all Day 59 contract lines, quality checklist entries, and delivery board tasks in docs.")

    if not failed and not critical_failures:
        wins.append("Day 59 Phase-3 pre-plan closeout lane is fully complete and ready for Day 60 execution lane.")
    ...
    return {
        ...
        "inputs": {
            ...
            "top10": _TOP10_PATH,
            "day58_summary": str(day58_summary.relative_to(root)) if day58_summary.exists() else str(day58_summary),
            "day58_delivery_board": str(day58_board.relative_to(root)) if day58_board.exists() else str(day58_board),
        },
        "checks": checks,
        "rollup": {"day58_activation_score": day58_score, "day58_checks": day58_check_count, "day58_delivery_board_items": board_count},
        "summary": {
...
def _emit_pack(root: Path, pack_dir: Path, payload: dict[str, Any]) -> None:
    ...
    _write(target / "day59-phase3-preplan-closeout-summary.json", json.dumps(payload, indent=2) + "\n")
    ...
    _write(target / "day59-delivery-board.md", "\n".join(["# Day 59 delivery board", *_REQUIRED_DELIVERY_BOARD_LINES]) + "\n")
    _write(target / "day59-validation-commands.md", "# Day 59 validation commands\n\n```bash\n" + "\n".join(_EXECUTION_COMMANDS) + "\n```\n")
...
def _execute_commands(root: Path, evidence_dir: Path) -> None:
    ...
    _write(out_dir / "day59-execution-summary.json", json.dumps({"total_commands": len(events), "commands": events}, indent=2) + "\n")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if ns.execute:
        evidence_dir = Path(ns.evidence_dir) if ns.evidence_dir else Path("docs/artifacts/day59-phase3-preplan-closeout-pack/evidence")
