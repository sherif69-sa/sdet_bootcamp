...
def build_day87_governance_handoff_closeout_summary(root: Path) -> dict[str, Any]:
    ...
    day86_summary_data = day86_data.get("summary", {}) if isinstance(day86_data.get("summary"), dict) else {}
    day86_score = int(day86_summary_data.get("activation_score", 0) or 0)
    day86_strict = bool(day86_summary_data.get("strict_pass", False))
    day86_check_count = len(day86_data.get("checks", [])) if isinstance(day86_data.get("checks"), list) else 0
    ...
    checks: list[dict[str, Any]] = [
        {"check_id": "readme_day87_command", "weight": 7, "passed": ("day87-governance-handoff-closeout" in readme_text), "evidence": "README day87 command lane"},
        {
            "check_id": "docs_index_day87_links",
            "weight": 8,
            "passed": ("day-87-big-upgrade-report.md" in docs_index_text and "integrations-day87-governance-handoff-closeout.md" in docs_index_text),
            "evidence": "day-87-big-upgrade-report.md + integrations-day87-governance-handoff-closeout.md",
        },
        {"check_id": "top10_day87_alignment", "weight": 5, "passed": ("Day 86" in top10_text and "Day 87" in top10_text), "evidence": "Day 86 + Day 87 strategy chain"},
        {"check_id": "day86_summary_present", "weight": 10, "passed": day86_summary.exists(), "evidence": str(day86_summary)},
        {"check_id": "day86_delivery_board_present", "weight": 7, "passed": day86_board.exists(), "evidence": str(day86_board)},
        {
            "check_id": "day86_quality_floor",
            "weight": 13,
            "passed": day86_score >= 85 and day86_strict,
            "evidence": {"day86_score": day86_score, "strict_pass": day86_strict, "day86_checks": day86_check_count},
        },
        {
    ...
    if day86_score >= 85 and day86_strict:
        ...
        handoff_actions.append("Re-run Day 86 closeout command and raise baseline quality above 85 with strict pass before Day 87 lock.")

    if board_count >= 5 and board_has_day86:
        wins.append(f"Day 86 delivery board integrity validated with {board_count} checklist items.")
    else:
        misses.append("Day 86 delivery board integrity is incomplete (needs >=5 items and Day 86 anchors).")
    ...
    if not missing_plan_keys:
        ...
        handoff_actions.append("Update .day87-governance-handoff-plan.json to restore required keys.")

    if not failed and not critical_failures:
        wins.append("Day 87 governance handoff closeout lane is fully complete and ready for Day 88 governance planning.")
    ...
    return {
        ...
        "inputs": {
            ...
            "top10": _TOP10_PATH,
            "day86_summary": str(day86_summary.relative_to(root)) if day86_summary.exists() else str(day86_summary),
            "day86_delivery_board": str(day86_board.relative_to(root)) if day86_board.exists() else str(day86_board),
            "launch_readiness_plan": _PLAN_PATH,
        },
        "checks": checks,
        "rollup": {"day86_activation_score": day86_score, "day86_checks": day86_check_count, "day86_delivery_board_items": board_count},
        "summary": {
...
def _emit_pack(root: Path, pack_dir: Path, payload: dict[str, Any]) -> None:
    ...
    _write(target / "day87-governance-handoff-closeout-summary.json", json.dumps(payload, indent=2) + "\n")
    ...
    _write(target / "day87-narrative-template-upgrade-ledger.json", json.dumps({"upgrades": []}, indent=2) + "\n")
    _write(target / "day87-storyline-outcomes-ledger.json", json.dumps({"outcomes": []}, indent=2) + "\n")
    ...
    _write(target / "day87-delivery-board.md", "\n".join(["# Day 87 delivery board", *_REQUIRED_DELIVERY_BOARD_LINES]) + "\n")
    _write(target / "day87-validation-commands.md", "# Day 87 validation commands\n\n```bash\n" + "\n".join(_EXECUTION_COMMANDS) + "\n```\n")
...
def _execute_commands(root: Path, evidence_dir: Path) -> None:
    ...
    _write(out_dir / "day87-execution-summary.json", json.dumps({"total_commands": len(events), "commands": events}, indent=2) + "\n")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if ns.execute:
        evidence_dir = Path(ns.evidence_dir) if ns.evidence_dir else Path("docs/artifacts/day87-governance-handoff-closeout-pack/evidence")
