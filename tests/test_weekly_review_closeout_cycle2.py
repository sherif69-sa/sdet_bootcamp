...
def _seed_repo(root: Path) -> None:
    ...
    (root / "README.md").write_text(
        "docs/integrations-weekly-review-closeout-cycle2.md\nweekly-review-closeout-cycle2\n",
        encoding="utf-8",
    )
    (root / "docs").mkdir(parents=True, exist_ok=True)
    (root / "docs/index.md").write_text(
        "impact-65-big-upgrade-report.md\nintegrations-weekly-review-closeout-cycle2.md\n",
        encoding="utf-8",
    )
    ...
    (root / "docs/integrations-weekly-review-closeout-cycle2.md").write_text(
        d65._DAY65_DEFAULT_PAGE, encoding="utf-8"
    )
    (root / "docs/impact-65-big-upgrade-report.md").write_text("# Day 65 report\n", encoding="utf-8")
...
def test_day65_cli_dispatch(tmp_path: Path, capsys) -> None:
    ...
    assert rc == 0
    alias_rc = cli.main(
        ["weekly-review-closeout-cycle2", "--root", str(tmp_path), "--format", "text"]
    )
    assert alias_rc == 0
