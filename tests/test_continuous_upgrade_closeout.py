...
def _seed_repo(root: Path) -> None:
    ...
    (root / "README.md").write_text(
        "docs/integrations-continuous-upgrade-closeout.md\nday91-continuous-upgrade-closeout\n",
        encoding="utf-8",
    )
    ...
    (root / "docs/impact-91-big-upgrade-report.md").write_text(
        "# Day 91 report\n", encoding="utf-8"
    )
    ...
    plan = root / "docs/roadmap/plans/continuous-upgrade-plan.json"
    plan.write_text(
        json.dumps(
            {
...
def test_day91_json(tmp_path: Path, capsys) -> None:
    ...
    assert out["name"] == "continuous-upgrade-closeout"
