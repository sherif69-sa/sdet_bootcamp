...
def _seed_repo(root: Path) -> None:
    ...
    (root / "docs/index.md").write_text(
        "continuous-upgrade-cycle9-big-upgrade-report.md\nintegrations-continuous-upgrade-cycle9-closeout.md\n",
        encoding="utf-8",
    )
    ...
    plan = root / "docs/roadmap/plans/continuous-upgrade-cycle9-plan.json"
    plan.write_text(
        json.dumps(
            {
...
def test_cycle9_json(tmp_path: Path, capsys) -> None:
    ...
    assert out["name"] == "continuous-upgrade-cycle9-closeout"
...
def test_cycle9_emit_pack_and_execute(tmp_path: Path) -> None:
    ...
    rc = d99.main(
        [
            ...
            str(tmp_path),
            "--emit-pack-dir",
            "artifacts/cycle9-pack",
            "--execute",
            "--evidence-dir",
            "artifacts/cycle9-pack/evidence",
            "--format",
            "json",
            "--strict",
        ]
    ...
    assert rc == 0
    assert (
        tmp_path / "artifacts/cycle9-pack/continuous-upgrade-cycle9-closeout-summary.json"
    ).exists()
    assert (
...
def test_cycle9_cli_dispatch(tmp_path: Path, capsys) -> None:
    _seed_repo(tmp_path)
    rc = cli.main(
        ["continuous-upgrade-cycle9-closeout", "--root", str(tmp_path), "--format", "text"]
    )
    assert rc == 0
