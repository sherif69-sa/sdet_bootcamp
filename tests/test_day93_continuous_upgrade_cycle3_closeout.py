...
def _seed_repo(root: Path) -> None:
    ...
    board = (
        root / "docs/artifacts/day92-continuous-upgrade-cycle2-closeout-pack/day92-delivery-board.md"
    )
    board.write_text(
        "\n".join(
            [
...
def test_day93_json(tmp_path: Path, capsys) -> None:
    ...
    assert out["name"] == "day93-continuous-upgrade-cycle3-closeout"
...
def test_day93_emit_pack_and_execute(tmp_path: Path) -> None:
    ...
    assert (tmp_path / "artifacts/day93-pack/day93-continuous-upgrade-cycle3-closeout-summary.md").exists()
...
def test_day93_cli_dispatch(tmp_path: Path, capsys) -> None:
    _seed_repo(tmp_path)
    rc = cli.main(
        ["day93-continuous-upgrade-cycle3-closeout", "--root", str(tmp_path), "--format", "text"]
    )
    assert rc == 0
