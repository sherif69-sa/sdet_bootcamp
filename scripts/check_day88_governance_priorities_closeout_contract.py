...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Validate Day 88 governance priorities closeout contract"
    )
    ...
    if not ns.skip_evidence:
        evidence = (
            root
            / "docs/artifacts/day88-governance-priorities-closeout-pack/evidence/day88-execution-summary.json"
        )
    ...
    if errors:
        print("day88-governance-priorities-closeout contract check failed:", file=sys.stderr)
        for error in errors:
            print(f"- {error}", file=sys.stderr)
        return 1

    print("day88-governance-priorities-closeout contract check passed")
    return 0
