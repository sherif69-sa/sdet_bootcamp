...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Validate Cycle 10 continuous upgrade closeout contract"
    )
    ...
    payload = d100.build_continuous_upgrade_cycle10_closeout_summary(root)
    errors: list[str] = []
    ...
    if not ns.skip_evidence:
        evidence = (
            root
            / "docs/artifacts/continuous-upgrade-cycle10-closeout-pack/evidence/cycle10-execution-summary.json"
        )
    ...
    if errors:
        print("continuous-upgrade-cycle10-closeout contract check failed:", file=sys.stderr)
        for error in errors:
            print(f"- {error}", file=sys.stderr)
        return 1

    print("continuous-upgrade-cycle10-closeout contract check passed")
    return 0
