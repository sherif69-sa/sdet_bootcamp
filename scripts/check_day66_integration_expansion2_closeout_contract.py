...
def main() -> int:
    parser = argparse.ArgumentParser(description="Validate Day 66 integration expansion #2 closeout contract")
    ...
    if not ns.skip_evidence:
        evidence = (
            root
            / "docs/artifacts/day66-integration-expansion2-closeout-pack/evidence/day66-execution-summary.json"
        )
    ...
    if errors:
        print("day66-integration-expansion2-closeout contract check failed:", file=sys.stderr)
        for err in errors:
            print(f"- {err}", file=sys.stderr)
        return 1

    print("day66-integration-expansion2-closeout contract check passed")
    return 0
