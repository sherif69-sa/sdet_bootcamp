...
def main() -> int:
    ...
    payload = kpa.build_kpi_audit_summary(root)

    strict_failures: list[str] = []
    page = root / kpa._PAGE_PATH
    page_text = page.read_text(encoding="utf-8") if page.exists() else ""
    for section in [kpa._SECTION_HEADER, *kpa._REQUIRED_SECTIONS]:
        if section not in page_text:
            strict_failures.append(section)
    for command in kpa._REQUIRED_COMMANDS:
    ...
    if not ns.skip_evidence:
        evidence = root / "docs/artifacts/day27-kpi-pack/evidence/day27-execution-summary.json"
