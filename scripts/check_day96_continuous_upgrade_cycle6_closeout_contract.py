...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Validate Day 96 continuous upgrade closeout contract"
    )
    ...
    payload = d93.build_day96_continuous_upgrade_cycle6_closeout_summary(root)
    errors: list[str] = []
    ...
    if not ns.skip_evidence:
        evidence = (
            root
            / "docs/artifacts/day96-continuous-upgrade-cycle6-closeout-pack/evidence/day96-execution-summary.json"
        )
    ...
    if errors:
        print("day96-continuous-upgrade-cycle6-closeout contract check failed:", file=sys.stderr)
        for error in errors:
            print(f"- {error}", file=sys.stderr)
        return 1

    print("day96-continuous-upgrade-cycle6-closeout contract check passed")
    return 0
