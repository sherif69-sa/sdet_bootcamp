...
def main() -> int:
    parser = argparse.ArgumentParser(description="Validate Day 68 integration expansion #4 closeout contract")
    ...
    if not ns.skip_evidence:
        evidence = (
            root
            / "docs/artifacts/day68-integration-expansion4-closeout-pack/evidence/day68-execution-summary.json"
        )
    ...
    if errors:
        print("day68-integration-expansion4-closeout contract check failed:", file=sys.stderr)
        for err in errors:
            print(f"- {err}", file=sys.stderr)
        return 1

    print("day68-integration-expansion4-closeout contract check passed")
    return 0
